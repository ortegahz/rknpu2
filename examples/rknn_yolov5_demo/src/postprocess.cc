...
int loadLabelName(const char* locationFilename, char* label[])
{
  ...
  readLines(locationFilename, label, OBJ_CLASS_NUM_BHV);
  return 0;
}
...
static int process_acfree_bhv(int8_t* input_c, int8_t* input_b, int grid_h, int grid_w, int height, int width, int stride,
                   std::vector<float>& boxes, std::vector<float>& objProbs, std::vector<int>& classId, float threshold,
                   int32_t zp_c, float scale_c, int32_t zp_b, float scale_b)
{
  int    validCount = 0;
  ...
  for (int i = 0; i < grid_h; i++) {
    for (int j = 0; j < grid_w; j++) {
          ...
          int8_t maxClassProbs = in_ptr_c[0];
          int    maxClassId    = 0;
          for (int k = 1; k < OBJ_CLASS_NUM_BHV; ++k) {
            int8_t prob = in_ptr_c[(0 + k) * grid_len];
            if (prob > maxClassProbs) {
              maxClassId    = k;
              maxClassProbs = prob;
                 ...
                 std::vector<float>& qnt_scales, detect_result_group_t* group)
{
  ...
  int grid_w0     = model_in_w / stride0;
  int validCount0 = 0;
  validCount0 = process_acfree_bhv(input0, input3, grid_h0, grid_w0, model_in_h, model_in_w, stride0, filterBoxes, objProbs,
                        classId, conf_threshold, qnt_zps[12], qnt_scales[12], qnt_zps[15], qnt_scales[15]);
  ...
  int grid_w1     = model_in_w / stride1;
  int validCount1 = 0;
  validCount1 = process_acfree_bhv(input1, input4, grid_h1, grid_w1, model_in_h, model_in_w, stride1, filterBoxes, objProbs,
                        classId, conf_threshold, qnt_zps[13], qnt_scales[13], qnt_zps[16], qnt_scales[16]);
  ...
  int grid_w2     = model_in_w / stride2;
  int validCount2 = 0;
  validCount2 = process_acfree_bhv(input2, input5, grid_h2, grid_w2, model_in_h, model_in_w, stride2, filterBoxes, objProbs,
                        classId, conf_threshold, qnt_zps[14], qnt_scales[14], qnt_zps[17], qnt_scales[17]);
